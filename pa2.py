			...
			if words[0].lower() == "create": # found the command create
				if words[1].lower() == "database":
					...
					if currentDb == "NA":
						...
						if not tb_exists(words[2], currentDb):
				...
				if words[1].lower() == "table":
					...
					elif tb_exists(words[2], currentDb):
						currentTable = Table(words[2], currentDb)
						currentTable.drop()
				...
				elif tb_exists(words[3], currentDb): # add words to select list until word = from
				...
				if words[1].lower() == "table":
					if currentDb == "NA":
						...
						if tb_exists(words[2], currentDb):
							currentTable = Table(words[2], currentDb)
							if words[5] == "int":
